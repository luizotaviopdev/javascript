...
function executar(){
    ...
    if (prod === '' || preco == '' || preco <= 0) {
      window.alert('Por favor, preencha os campos com um número válido maior do que 0')
    } else {
      let desconto = preco * 0.10
      
      saida.innerHTML = `O valor do produto era ${preco.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}. <br>
      Você acaba de ganhar ${desconto.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})} de desconto (-10%). <br>
      No fim, você vai pagar ${(preco - desconto).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})} no produto ${prod}`
    }
}