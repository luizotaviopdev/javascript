...
function executar() {
  ...
  if (a === '' || b === '' || c === '') {
    window.alert('Por favor, preencha os campos com números válidos')
    return;
  }

  if (a == 0) {
    window.alert('O valor de A em uma equação de segundo grau não pode ser 0, corrigimos automaticamente para 1')
    a = 1
    document.getElementById('a').value = 1
  }

  let delta = b ** 2 - 4 * a * c;
  let raizmais = (-b + Math.sqrt(delta)) / (2 * a);
  let raizmenos = (-b - Math.sqrt(delta)) / (2 * a);

  saida.innerHTML = `O valor de delta é ${delta}. <br>
  O valor da raiz de mais é ${raizmais} <br>
  E o valor da raiz de menos é ${raizmenos} <br>`
}