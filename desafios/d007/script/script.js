...
document.querySelectorAll('#dol, #rs').forEach(element => {
...
function executar(){
    ...
    if (dol === '' || dol <= 0 || rs === '' || rs <= 0) {
      window.alert('Por favor, preencha os campos com um número válido maior do que 0')
    } else {
      let valdol = rs / dol
      saida.innerHTML = `Você pode comprar ${(valdol).toLocaleString('en-US', {style: 'currency', currency: 'USD'} )}`
    }
}